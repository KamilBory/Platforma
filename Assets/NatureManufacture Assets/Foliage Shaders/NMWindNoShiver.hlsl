...
sampler WIND_SETTINGS_TexNoise;
sampler WIND_SETTINGS_TexGust;
...
float3 RotateNM(float3 pivot, float3 position, float3 rotationAxis, float angle)
{
                            ...
                            float          time)
{
    ...
    if (wind.Strength > 0.0)
    {
        ...
        positionWS = RotateNM(rootWP, positionWS, rotAxis, (wind.Strength) * 0.001 * att);
